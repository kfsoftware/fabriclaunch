	...
	return (
		<button
			...
			className={clsx(
				...
				copied ? 'bg-emerald-400/10 ring-1 ring-inset ring-emerald-400/20' : 'bg-white/5 hover:bg-white/7.5 dark:bg-white/2.5 dark:hover:bg-white/5'
			)}
			onClick={() => {
...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
	...
	return (
		<div
			className={`flex h-9 items-center gap-2 border-y border-b-white/7.5 border-t-transparent bg-white/2.5  px-4 dark:border-b-white/5 dark:bg-white/1
		...
		return (
			<div className="group dark:bg-white/2.5">
				...
				<div className="relative">
					<pre className="overflow-x-auto p-4 text-xs text-white"></pre>
					<CopyButton code={''} />
				</div>
			</div>
		)
...
export function CodeRaw({ code, lang }: { code: string; lang: string }) {
...
export function CodePanelRaw({ tag, label, code, lang }: { lang: string; tag?: string; label?: string; code?: string }) {
	const [highlightedCode, setHighlightedCode] = useState('')
...
export const CodePanelComponent = CodePanel
