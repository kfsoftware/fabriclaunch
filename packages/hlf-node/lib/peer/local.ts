...
export class LocalPeer {
  ...
  async init(): Promise<PeerConfig> {
    ...
    const tlsCerts = await this.org.getCertificateForNode(peerId, {
      dnsNames,
      ipAddresses,
      organization: this.org.mspId,
      organizationUnit: 'peer'
    }, 'tls');
    const signCerts = await this.org.getCertificateForNode(peerId, {
      dnsNames: [],
      ipAddresses: [],
      organization: this.org.mspId,
      organizationUnit: 'peer'
    ...
    await fs.writeFile(buildBuilderPath, `#!/bin/sh
...
`, { encoding: "utf8" })
    ...
    await fs.writeFile(detectBuilderPath, `#!/bin/sh
...
exit 1
    ...
    await fs.writeFile(releaseBuilderPath, `#!/bin/sh
...
`
  ...
  async start(): Promise<StartCmdResponse | StartSystemdResponse | StartDockerResponse> {
    ...
    switch (this.mode) {
      ...
      case "docker": {
        try {
          const envArrayCmd = Object.entries(env)
          ...
          const p = Bun.spawnSync({
            cmd: [
              ...
              '-v', `${dataConfigPath}:/var/hyperledger/production`,
              '-p', `${this.opts.listenAddress.split(':')[1]}:7051`,
              '-p', `${this.opts.chaincodeAddress.split(':')[1]}:7052`,
              '-p', `${this.opts.eventsAddress.split(':')[1]}:7053`,
              '-p', `${this.opts.operationsListenAddress.split(':')[1]}:9443`,
          ...
          // console.log(`Started peer container: ${this.containerName}`);
          return {
            mode: "docker",
            containerName,
          }
  ...
  private get serviceName(): string {
    return `fabric-peer-${slugify(this.opts.id)}.service`;
  }
  ...
  private async createSystemdService(): Promise<void> {
    const homeDir = os.homedir();
    const dirPath = path.join(homeDir, `.fabriclaunch/peers/${slugify(this.opts.id)}`);
    const mspConfigPath = path.join(dirPath, 'config');
    const cmd = this.buildPeerCommand();
    const env = this.buildPeerEnvironment(mspConfigPath);
