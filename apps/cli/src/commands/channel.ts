...
export class ChannelCommands {
	...
	async accept(
		...
		const { signature, cert } = await localOrg.signDataAsAdmin(Buffer.from(proposal.channelData.channelTx, 'base64'))
	...
	async join(
		...
		await localOrg.prepareAdminCertMSP(adminMSPPath.name)
	...
	@Command({
		name: 'set-anchor-peers',
		description: 'Set anchor peers for a channel',
	})
	async setAnchorPeers(
		@Arg({ name: 'channelName', description: 'Name of the channel' })
		channelName: string,
		@Flag({ name: 'org', alias: 'o', description: 'Organization to set anchor peers for', type: 'string', required: true })
		@Flag({ name: 'peer', alias: 'p', description: 'Peer to use for the channel', type: 'string', required: true })
		@Flag({ name: 'peerAddresses', alias: 'p', description: 'Peer addresses to set as anchor peers', type: 'string[]', required: true })
		@Flag({ name: 'ordererUrl', description: 'Orderer URL', type: 'string', required: true })
		@Flag({ name: 'ordererTlsCaCert', description: 'Orderer TLS CA cert (base64 encoded)', type: 'string', required: true })
		flags: {
			org: string
			peer: string
		...
		const spinner = ora('Setting anchor peers').start()
		if (!(await storage.checkIfLoggedIn())) {
		...
		try {
			...
			spinner.fail(`Org ${flags.org} does not exist`)
			return
		}
		...
		await localOrg.prepareAdminCertMSP(adminMSPPath.name)
		const homeDir = os.homedir()
		const slugifiedId = slugify(flags.peer)
		const dirPath = path.join(homeDir, `.fabriclaunch/peers/${slugifiedId}`)
		const mspConfigPath = path.join(dirPath, 'config')
		const envVariables = {
			FABRIC_CFG_PATH: mspConfigPath,
			CORE_PEER_ADDRESS: flags.peerAddresses[0],
			CORE_PEER_LOCALMSPID: flags.org,
			CORE_PEER_TLS_ENABLED: 'true',
			FABRIC_LOGGING_SPEC: 'fatal',
			CORE_PEER_MSPCONFIGPATH: adminMSPPath.name,
		...
		// Fetch config block
		spinner.text = 'Fetching config block'
		const { path: configBlockFileTmp } = createTempFileSync()
		try {
			await $`peer channel fetch config -o ${flags.ordererUrl} -c ${channelName} --tls --cafile ${flags.ordererTlsCaCert} ${configBlockFileTmp}`.env(envVariables)
		} catch (e) {
			console.log('error', e)
			spinner.fail(`Failed to fetch channel ${channelName}: ${(e as ShellError).stderr.toString('utf-8')}`)
			return
		}

		// Update config with anchor peers
		spinner.text = 'Updating config with anchor peers'
		const anchorPeers = flags.peerAddresses.map((peer) => ({
			host: peer.split(':')[0],
		...
		if (updateConfigRes.errors && updateConfigRes.errors.length > 0) {
			spinner.fail(updateConfigRes.errors[0].message)
			return
		}

		if (updateConfigRes.data?.updateConfigWithAnchorPeers.noChanges) {
			spinner.succeed(`Anchor peers already set`)
			return
		}

		// Sign and submit updated config
		spinner.text = 'Signing and submitting updated config'
		const { path: updateConfigEnvelopeTmp } = createTempFileSync()
		await fs.writeFile(updateConfigEnvelopeTmp, Buffer.from(updateConfigRes.data!.updateConfigWithAnchorPeers.updateB64, 'base64'))

		try {
			spinner.text = `Signing updated config`
			await $`peer channel signconfigtx -f ${updateConfigEnvelopeTmp}`.env(envVariables).quiet()
			spinner.text = `Updating channel ${channelName}`
			await $`peer channel update -f ${updateConfigEnvelopeTmp} -c ${channelName} -o ${flags.ordererUrl} --tls --cafile ${flags.ordererTlsCaCert}`.env(envVariables).quiet()
			spinner.succeed(`Anchor peers set successfully`)
		} catch (e) {
			spinner.fail(`Failed to update the channel: ${(e as ShellError).stderr.toString('utf-8')}`)
			return
		}
	}
}
